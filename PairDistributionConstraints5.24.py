...
pdc = PairDistributionConstraint(
    experimentalData="pdf.dat", 
    weighting="xrays_scattering",  # 使用X射线散射因子
    qRange=q_range      # 指定Q值范围

window_function = make_lorch_window(Qmax, dr, half_width_points=120)
...
class PairDistributionConstraint(ExperimentalConstraint):
    ...
    **NB**: If adjustScaleFactor first item (frequency) is 0, the scale factor
    ...
    def __get_total_Gr(self, data, rho0):
        ...
        # 替换标准卷积为镜像卷积 - 直接在这里实现，不使用自定义类
        if self.__windowFunction is not None:
            # 镜像扩展 G(r)
            Gr_mirrored = np.concatenate((-Gr[::-1], Gr))
        
            # 执行卷积
            full_conv = np.convolve(Gr_mirrored, self.__windowFunction, 'same')
    ...
    def _get_constraint_value(self, data, applyMultiframePrior=True):
        ...
        if self.__windowFunction is not None:
            # 镜像扩展 G(r)
            total_mirrored = np.concatenate((-output["total_no_window"][::-1], output["total_no_window"]))
        
            # 执行卷积
            full_conv = np.convolve(total_mirrored, self.__windowFunction, 'same')
        
            # 只保留对应于原始 G(r) 的部分
            output["total"] = full_conv[len(output["total_no_window"]):]
        else:
