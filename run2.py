...
class SimulationManager:
    ...
    def _setup_atom_groups(self):
        ...
        try:
            ...
            def create_sub_groups(element, indexes, sub_group_size):
                sub_groups = [indexes[i:i + sub_group_size] for i in range(0, len(indexes), sub_group_size)]
                for i, sub_group in enumerate(sub_groups):
                    generator = MDAwareMoveGenerator(
                        amplitude=0.40 if element == 'H' else 0.25,  # 初期设置
                        elementParameters=self.element_parameters,
                        temperature=self.initial_temperature
                    )
                    sub_group_name = f"{element}_Group_{i+1}"
                    group = Group(indexes=sub_group, name=sub_group_name)
                    group.set_move_generator(generator)
                    groups.append(group)
                    logger.info(f"创建{element}原子子组{sub_group_name}，包含{len(sub_group)}个原子")

            # 创建各元素的子组
            for element in ['C', 'H', 'O', 'N']:
                if element in element_groups:
                    create_sub_groups(element, element_groups[element], sub_group_size=15)
    ...
    def run_simulation(self, nsteps=30000, saveFrequency=10000, xyzFrequency=10000):
        ...
        try:
            ...
            # 引入 TranslationGenerator 和 RotationGenerator
            translation_generator = TranslationGenerator(amplitude=0.1)
            rotation_generator = RotationGenerator(amplitude=10.0)

            # 第1阶段: 初期探索
            logger.info("模拟第1阶段: 初期探索 (0-10000步)")
            total_steps = 0

            # 设置初期参数
            for group in self.engine.groups:
                if hasattr(group.moveGenerator, 'set_amplitude'):
                    group.moveGenerator.set_amplitude(0.40 if 'Hydrogen' in group.name else 0.25)
                if hasattr(group.moveGenerator, 'set_temperature'):
                    group.moveGenerator.set_temperature(self.initial_temperature)
                # 添加 Translation 和 Rotation 生成器
                if hasattr(group, 'name'):
                    if 'Hydrogen' in group.name:
                        group.set_move_generator(translation_generator)
                    else:
                        group.set_move_generator(rotation_generator)

            self.current_temperature = self.initial_temperature
            init_tolerances = {k: 0.1 for k, v in nmr_constraint.tolerances.items()}
            nmr_constraint.set_tolerances(init_tolerances)
            nmr_constraint.set_reject_probability(0.3)

            self.engine.run(numberOfSteps=10000, saveFrequency=1000)
            total_steps += 10000
            ...
            # 第2阶段: 中期优化
            logger.info("模拟第2阶段: 中期优化 (10000-20000步)")
            for group in self.engine.groups:
                if hasattr(group.moveGenerator, 'set_amplitude'):
                    group.moveGenerator.set_amplitude(0.32 if 'Hydrogen' in group.name else 0.20)
                if hasattr(group.moveGenerator, 'set_temperature'):
                    group.moveGenerator.set_temperature(350.0)

            self.current_temperature = 350.0
            ...
            # 第3阶段: 后期收敛
            logger.info("模拟第3阶段: 后期收敛 (20000-30000步)")
            for group in self.engine.groups:
                if hasattr(group.moveGenerator, 'set_amplitude'):
                    group.moveGenerator.set_amplitude(0.20 if 'Hydrogen' in group.name else 0.12)
                if hasattr(group.moveGenerator, 'set_temperature'):
                    group.moveGenerator.set_temperature(250.0)

            self.current_temperature = 250.0
            ...
            data, phase3_error = nmr_constraint.compute_data(update=True)
            logger.info(f"第3阶段接受率: {phase3_accept_rate:.4f} ({self.engine.accepted}/{self.engine.tried})")
            logger.info(f"第3阶段误差: {phase3_error:.6f} (初始误差的 {phase3_error/initial_error:.2f})")
