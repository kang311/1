...
# -*- coding: utf-8 -*-
"""
MDHRMCrun.py - 混合分子动力学和杂化反向蒙特卡罗模拟运行脚本

集成分子动力学(MD)和杂化反向蒙特卡罗(HRMC)来优化原子结构，
匹配实验g(r)数据并保持合理的化学结构。
"""
...
from fullrmc.MDHRMCEngine import MDHRMCEngine  # 导入我们的自定义引擎
...
# 命令行参数解析
parser = argparse.ArgumentParser(description='运行混合MD-HRMC模拟')
...
parser.add_argument('--md_temperature', type=float, default=800, help='MD温度(K)')
parser.add_argument('--md_steps', type=int, default=100, help='每个周期的MD步数')
parser.add_argument('--hrmc_steps', type=int, default=500, help='每个周期的HRMC步数')
parser.add_argument('--hrmc_initial_temp', type=float, default=2000, help='HRMC初始温度(K)')
parser.add_argument('--hrmc_final_temp', type=float, default=300, help='HRMC最终温度(K)')
parser.add_argument('--cycles', type=int, default=1, help='MD-HRMC循环周期数')
args = parser.parse_args()

# 设置路径
CURRENT_DIR = os.getcwd()
DATE_TAG = datetime.now().strftime('%Y%m%d_%H%M%S')
OUTPUT_DIR = args.output_dir if args.output_dir else os.path.join(CURRENT_DIR, f"mdhrmc_{DATE_TAG}")
...
ENGINE_PATH = os.path.join(OUTPUT_DIR, "md_hrmc_engine.rmc")

def main():
    """主模拟流程"""
    logger.info(f"启动混合MD-HRMC模拟")
    ...
    if os.path.exists(ENGINE_PATH) and args.restart:
        # 加载现有引擎
        ENGINE = MDHRMCEngine(path=ENGINE_PATH)
    ...
    if not args.restart or not os.path.exists(ENGINE_PATH):
        ...
        max_translation = 0.2
        logger.info(f"盒子尺寸: {box_lengths}, 最大移动振幅: {max_translation}")
        
        # 所有要调整的元素
        elements_constraint = ['C', 'H', 'O', 'N', 'S']
        ...
        # 创建平移生成器
        translation = TranslationGenerator(amplitude=0.2)
    ...
    # 运行MD-HRMC模拟
    try:
        logger.info("开始MD-HRMC模拟...")
        ENGINE.run_md_hrmc()
    except Exception as e:
        logger.error(f"MD-HRMC模拟失败: {str(e)}")
        raise
    ...
    # 获取最终PDF误差
    pdf_error = None
    if hasattr(ENGINE, 'pdf_constraint') and ENGINE.pdf_constraint:
        pdf_error = ENGINE.pdf_constraint.standardError
        logger.info(f"最终PDF误差: {pdf_error:.6f}")
    
    # 获取最终能量误差
    energy_error = None
    if hasattr(ENGINE, 'energy_constraint') and ENGINE.energy_constraint:
        energy_error = ENGINE.energy_constraint.standardError
        logger.info(f"最终能量误差: {energy_error:.6f}")
    
    # 输出总标准误差
    logger.info(f"总标准误差: {ENGINE.totalStandardError:.6f}")
