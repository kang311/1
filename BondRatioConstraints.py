...
class BondRatioConstraint(Constraint):
    ...
    def __init__(self, targetBondRatios=None, weights=None):
        # 初始化基类
    ...
    def set_weights(self, weights=None):
        ...
        # 记录到存储库
        self._dump_to_repository({'_BondRatioConstraint__weights': self.__weights})
    ...
    def compute_standard_error(self, modelData):
        ...
        """
        # 初始化计算所需变量
        numerator = FLOAT_TYPE(0.0)     # 加权平方差（分子）
        denominator = FLOAT_TYPE(0.0)   # 权重总和（分母）
        ...
        for bond_type, target in self.__targetBondRatios.items():
            ...
            # 累加加权平方差
            numerator += weight * ((current - target) ** 2)
        ...
        # 计算加权平均平方误差（χ²_bond）
        if denominator > 0:
            chi_square_bond = numerator / denominator
        else:
            # 如果没有有效权重，抛出错误
            raise ValueError("键比例权重总和为零，无法计算标准误差")
    
        return chi_square_bond
