...
class MDHRMCEngine(Engine):
        ...
        try:
            for step in range(numberOfSteps):
                ...
                try:
                    ...
                    error_msg = str(e)
                    # 检查是否是ReaxFF的bondchk错误
                    if "bondchk failed" in error_msg or "malformed" in error_msg:
                        LOGGER.warn(f"原子移动导致原子重叠或距离过近: {error_msg}")
                        # 捕获到重叠错误 - 自动拒绝移动
                        reject_move = True
                        # 恢复原始坐标
    ...
    def should_step_get_rejected(self, realIndexes, relativeIndexes):
        ...
        try:
            ...
            dis_old_error = self.dis_constraint.standardError
            dis_new_error = self.dis_constraint.afterMoveStandardError
            delta_chi_dis = dis_new_error - dis_old_error
            ...
            delta_chi_u = delta_U / self.omega
            ...
            if is_hydrogen:
                ...
                delta = (delta_chi_u) / T_chi
            else:
                # 对非H原子: P = min[1, exp(-(Δχ² + ΔU/ω)/T_χ)]
                delta = (delta_chi_sq + delta_chi_u) / T_chi

            # 处理数值溢出
            if delta > 700:
                self._write_data('HRMC_acceptance.txt',
                    f"{self._tried} 0.0 {delta_chi_sq:.6f} {delta_chi_u:.6f}  {delta_U:.6f} {T_chi:.6f}")
