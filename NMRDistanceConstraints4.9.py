...
class NMRDistanceConstraint(Constraint):
    ...
    def __init__(self, targetBondRatios=None, weights=None, varianceSquared=0.001):
        # 初始化基类
        super(NMRDistanceConstraint, self).__init__()
        
        # 添加类名属性以便于引擎自动识别
        self.name = "NMRDistanceConstraint"
    ...
    def compute_standard_error(self, modelData):
        ...
        try:
            ...
            for bond_type, target in self.__targetBondRatios.items():
                ...
                # 计算差值的平方
                squared_error = (current - target) ** 2
    ...
    def plot(self, filename=None, figsize=(14, 10), dpi=300):
        ...
        try:
            ...
            for i, bond in enumerate(bond_types):
                ...
                diff_percent = (diff / max(target_values[i], 1e-10)) * 100
                differences.append((diff, diff_percent))
            ...
            # 根据偏差程度使用不同颜色
            colors = []
            for diff, _ in differences:
                if abs(diff) < 0.05:  # 小于5%的偏差
                    colors.append('limegreen')
                elif abs(diff) < 0.1:  # 5%-10%的偏差
            ...
            # 计算符合率（偏差小于5%）
            compliant_count = sum(1 for diff, _ in differences if abs(diff) < 0.05)
            compliance_rate = compliant_count / len(bond_types) * 100
            
            # 设置标题，包含标准误差和符合率
            title = f"NMR Bond Analysis (StdErr: {self.standardError:.4f}, Compliance: {compliance_rate:.1f}%)"
            ...
            for i, v in enumerate(current_values):
                diff, diff_percent = differences[i]
                label = f"{v:.3f}\n{diff_percent:+.1f}%"
                ax1.text(i + width/2, v, label, ha='center', va='bottom' if diff_percent > 0 else 'top',
            ...
            table_text += f"Compliance Rate: {compliance_rate:.1f}%\n\n"
