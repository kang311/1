...
class MDHRMCEngine(Engine):
    ...
    def initialize_used_constraints(self):
        ...
        try:
            ...
            # Verify our specific constraints
            if not hasattr(self, 'pdf_constraint') or self.pdf_constraint is None:
                raise ValueError("PDF constraint not found")
            if not hasattr(self, 'energy_constraint') or self.energy_constraint is None:
                raise ValueError("Energy constraint not found")
                
        except Exception as e:
            LOGGER.error(f"Error initializing constraints: {str(e)}")
            raise
    ...
    def run(self, numberOfSteps):
        ...
        try:
            for step in range(numberOfSteps):
                ...
                # Convert indexes to numpy arrays
                ...
                # Compute constraints before move
                ...
                # Compute constraints after move
                box_coords = transform_coordinates(
                    transMatrix=self.reciprocalBasisVectors,
                    coords=moved_positions
                )
    ...
    def should_step_get_rejected(self, realIndexes, relativeIndexes):
        """Implement MD-HRMC acceptance criterion."""
        try:
            # Get moved atom info
            moved_elem = self.allElements[realIndexes[0]]
            is_hydrogen = (moved_elem == 'H')
            
            # Get experimental G(r) data
            exp_data = self.pdf_constraint.experimentalData
            exp_r = exp_data[:, 0]  # r values
            exp_gr = exp_data[:, 1]  # G(r) values
            
            try:
                # Step 1: Get current state histogram data
                old_hist = self.pdf_constraint.data
                if old_hist is None:
                    raise ValueError("Current state histogram data is None")

                # Step 2: Compute new state histogram
                new_hist, _ = self.pdf_constraint.compute_data(update=False)
                if new_hist is None:
                    raise ValueError("Failed to compute new state histogram")
                
                # Verify histogram data format
                for hist in [old_hist, new_hist]:
                    if not isinstance(hist, dict):
                        raise ValueError("Histogram data must be a dictionary")
                    if "intra" not in hist or "inter" not in hist:
                        raise ValueError("Histogram missing required keys 'intra' and 'inter'")

                # Step 3: Convert histograms to G(r)
                rho0 = len(self.allElements) / self.volume
                
                old_gr = self.pdf_constraint._PairDistributionConstraint__get_total_Gr(
                    data=old_hist,
                    rho0=rho0
                )
                
                new_gr = self.pdf_constraint._PairDistributionConstraint__get_total_Gr(
                    data=new_hist,
                    rho0=rho0
                )

                # Calculate χ² for both states
                def compute_chi_square(sim_gr):
                    """Calculate χ² = Σ(sim-exp)²/Σ(exp²)"""
                    diff_squared = (sim_gr - exp_gr)**2  # (G(r)_sim - G(r)_exp)²
                    exp_squared_sum = np.sum(exp_gr**2)  # Σ(G(r)_exp²)
                    return np.sum(diff_squared) / exp_squared_sum  # Σ(sim-exp)²/Σ(exp²)
                    
                # Calculate Δχ² = χ_j² - χ_i²
                chi_sq_new = compute_chi_square(new_gr)  # χ_j²  
                chi_sq_old = compute_chi_square(old_gr)  # χ_i²
                delta_chi_sq = chi_sq_new - chi_sq_old
                
                # Get ReaxFF energy difference (ΔU)
                ...
                # Calculate current T_chi
                T_chi = self.T_chi_0 * (self.cooling_rate**self.current_cycle)
                
                # Calculate acceptance probability based on atom type
                if is_hydrogen:
                    # For H atoms: P = min[1, exp(-(ΔU/ω)/T_χ)]
                    delta = (delta_U / self.omega) / T_chi
                else:
                    # For non-H atoms: P = min[1, exp(-(Δχ² + ΔU/ω)/T_χ)]
                    delta = (delta_chi_sq + delta_U / self.omega) / T_chi

                # Handle numerical overflow BEFORE exponential
                if delta > 700:
                ...
                # Calculate acceptance probability with min[1, exp(-delta)]
                ...
                LOGGER.error(f"Error computing G(r): {str(gr_error)}")
                return True
    ...
    def compute_chi_square(self, simulated, experimental):
        """Compute χ² between simulated and experimental PDF."""
        diff = simulated - experimental
        return np.sum(diff**2 / experimental**2)

    def __initialize_lammps(self):
        """Initialize LAMMPS if needed."""
        if self.__lmp is None:
            self.__lmp = lammps(cmdargs=["-screen", "none", "-log", "none"])
            self.__initialized = False
    ...
    def run_md_hrmc(self):
        ...
        try:
            for cycle in range(1, self.cycles_md_hrmc + 1):
                ...
                # Make sure LAMMPS is initialized
                if self.__lmp is None:
                ...
                # Run HRMC
                LOGGER.info("Running HRMC simulation...")
                self.run(numberOfSteps=self.hrmc_steps)    

                # After each cycle, calculate and write G(r) data
                exp_data = self.pdf_constraint.experimentalData
                exp_r = exp_data[:, 0]
