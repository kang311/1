...
class BondRatioHRMCEngine(Engine):
    ...
    def initialize_used_constraints(self):
        ...
        try:
            ...
            # 使用与MDHRMCEngine相同的omega计算方法
            self.omega = (N_total**2 / max(1, N_nonH)) * 7 * 23.0609
            
            # 计算初始归一化温度
            self.T_chi_0 = self.KB * self.initial_temperature / self.omega
            
            LOGGER.info(f"结构加载: 总原子数={N_total}, 非氢原子数={N_nonH}")
            LOGGER.info(f"温度转换系数 omega = {self.omega}")
            LOGGER.info(f"初始归一化温度 T_chi_0 = {self.T_chi_0}")
                
        except Exception as e:
            LOGGER.error(f"初始化约束出错: {str(e)}")
            raise
            
    def setup_hrmc(self, initial_temp, final_temp, steps, cooling_rate=0.98):
        """
        设置HRMC参数。
        
        :Parameters:
            #. initial_temp (float): 初始退火温度 (K)
            #. final_temp (float): 最终退火温度 (K) 
            #. steps (int): HRMC模拟步数
        ...
        # 更新温度相关参数
        if hasattr(self, 'omega') and self.omega > 0:
            self.T_chi_0 = self.KB * initial_temp / self.omega
        
        # 更新移动生成器的温度
        self._update_generators_temperature(self.current_temperature)
        
        LOGGER.info(f"已设置HRMC参数: 初始温度={initial_temp}K, 最终温度={final_temp}K, 冷却率={cooling_rate}")
        ...
        try:
            ...
            for step in range(1, numberOfSteps + 1):
                ...
                if step % saveFrequency == 0 or step == numberOfSteps:
                    ...
                    LOGGER.info(f"- 当前温度: {self.current_temperature:.1f}K")
