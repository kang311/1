...
class EnergyConstraint(Constraint):
    ...
    def _initialize_lammps(self):
        ...
        try:
            ...
            self.__lmp = lammps(cmdargs=[
                "-sf", "kk",                          # 使用KOKKOS后缀
                "-pk", "kokkos", "0", "omp", str(nthreads), # KOKKOS OpenMP后端
                "-log", "lammps.log", 
                "-screen", logfile
            ])
            lmp = self.__lmp
            ...
            # KOKKOS包设置
            lmp.command(f"package kokkos 0 omp {nthreads}")
            lmp.command("suffix kk")  # 使用KOKKOS后缀
            ...
            # 在定义盒子后设置输出控制
            lmp.command("echo none")     # 禁用回显
            lmp.command("log none")      # 禁用日志记录
            lmp.command("thermo_style custom step pe") # 设置热力学输出
            lmp.command("thermo_modify flush no norm no") # 最小化输出
            lmp.command("thermo 0")      # 禁用热力学输出
            ...
            # 设置KOKKOS版本的ReaxFF力场和邻居设置
            LOGGER.info("使用KOKKOS设置ReaxFF...")
            lmp.command(f"pair_style reaxff/kk {self.__control_params} safezone 20 mincap 800")
            ...
            lmp.command("info kokkos")

            self.__initialized = True
            self.__cached_box = box_lengths.copy()
        
            LOGGER.info("LAMMPS KOKKOS初始化成功完成")

        except Exception as e:
            self.__initialized = False
            self.__lmp = None
            LOGGER.error(f"LAMMPS KOKKOS初始化失败: {str(e)}")
        ...
        try:
            ...
            for idx, (pos, elem) in enumerate(zip(wrapped_positions, elements), start=1):
                ...
                if not np.all(np.isfinite(pos)):
                    raise LAMMPSError(f"原子 {idx} 的位置无效: {pos}")
            ...
            if natoms != n_atoms:
                raise LAMMPSError(f"创建后原子数量不匹配: {natoms} != {n_atoms}")
            ...
            # 应用KOKKOS版本的QEQ
            lmp.command("fix QEQ all qeq/reaxff/kk 1 0.0 10.0 1e-6 reaxff maxiter 1000")
            lmp.command("run 0")  # 运行QEQ而不移动原子

            # 保持QEQ修复，然后进行KOKKOS加速的最小化
            lmp.command("min_style cg/kk")  # 使用KOKKOS版本的共轭梯度
            lmp.command("min_modify dmax 0.1 line quadratic")            
            lmp.command("minimize 0.0e-4 1.0e-4 0 0")
            ...
            LOGGER.error(f"KOKKOS能量计算失败: {str(e)}")
            raise LAMMPSError(str(e))

            # 🔧 增强的KOKKOS错误处理
            if "bondchk failed" in error_msg or "malformed" in error_msg:
                LOGGER.warn(f"KOKKOS ReaxFF bondchk错误 - 原子距离过近，拒绝移动: {error_msg}")
                # 返回一个很大的能量值，强制拒绝移动
                return float('inf'), positions
            elif "kokkos" in error_msg.lower():
                LOGGER.error(f"KOKKOS专用错误: {error_msg}")
                raise LAMMPSError(f"KOKKOS错误: {error_msg}")
            elif "reaxff/kk" in error_msg:
                LOGGER.error(f"KOKKOS ReaxFF错误: {error_msg}")
                raise LAMMPSError(f"KOKKOS ReaxFF错误: {error_msg}")
            else:
                LOGGER.error(f"KOKKOS能量计算失败: {error_msg}")
                raise LAMMPSError(str(e))
